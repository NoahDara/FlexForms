...
class TableConfig(BaseModel):
    ...
    class TableType(models.TextChoices):
        DYNAMIC_ROWS = "dynamic_rows", "Dynamic Rows"
        FIXED_GRID = "fixed_grid", "Fixed Grid"

    name = models.CharField(
        max_length=255,
        unique=True,
        help_text="Human-readable name for this table e.g. 'Employment History Table'",
    )

    description = models.TextField(
        blank=True,
        null=True,
        help_text="Optional description of what this table captures",
    )

    table_type = models.CharField(
        max_length=20,
        choices=TableType.choices,
        default=TableType.DYNAMIC_ROWS,
...
class TableColumn(BaseModel):
    ...
    table = models.ForeignKey(
        ...
        help_text="The table this column belongs to",
    )

    header = models.CharField(
        max_length=255,
        help_text="The column header label displayed to the user e.g. 'Company Name'",
    )
    ...
    column_type = models.CharField(
        ...
        help_text="The data type for cells in this column",
    )
    ...
    is_required = models.BooleanField(
        default=False,
        help_text="Whether cells in this column must be filled in",
    )
    ...
    data_source = models.ForeignKey(
        ...
        help_text="Required when column_type is dropdown or radio",
    )
...
class TableRow(BaseModel):
    ...
    table = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="rows",
        help_text="The fixed grid table this row belongs to",
    )

    row_label = models.CharField(
        max_length=255,
        help_text="The row label displayed on the left side e.g. 'Service Quality'",
    )

    order = models.PositiveIntegerField(
        default=0,
        help_text="Display order of this row within the table (top to bottom)",
    )
...
class TableCellConfig(BaseModel):
    ...
    table = models.ForeignKey(
        TableConfig,
        on_delete=models.CASCADE,
        related_name="cell_configs",
        help_text="The fixed grid table this cell config belongs to",
    )
    ...
    row = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="cell_configs",
        help_text="The row this cell sits in",
    )
    ...
    column = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="cell_configs",
        help_text="The column this cell sits in",
    )

    cell_type = models.CharField(
        max_length=20,
        choices=TableColumn.ColumnType.choices,
        blank=True,
        null=True,
        help_text=(
    ...
    data_source = models.ForeignKey(
        ...
        blank=True,
        related_name="cell_configs",
        help_text="Required when cell_type overrides to dropdown or radio",
    )

    class Meta(BaseModel.Meta):
        verbose_name = "Table Cell Configuration"
        verbose_name_plural = "Table Cell Configurations"
